...
PATH_FILE_RESULT = './data/data.csv'
DATABASE_NAME = './data/fifaComplete.db'
...
def create_data_set(data, target_file, complex=True):
    ...
    for l in output_result:
        all_players_in_wm = all_players_in_wm.union(set(l[4:len(l) - 1]))
    ...
    for l in output_result:
        ...
        final_output.append(first_part + new_player_part + result_part)
    ...
    with open(target_file, 'w', encoding='utf-8') as output:
        print('Write...')
        pd.DataFrame(final_output).drop([0, 1, 2], axis=1).to_csv(output, header=False, encoding='utf-8', index=False)
...
def create_test_data_set(complex=True):
    ...
    """
    # get quaterfinals
    game_ids = ["2451968", "2451967", "2451969", "2451970"]
    table_game_data = 'wm2018gamedata'
    ...
    for l in data:
        ...
        _data[len(_data) - 1][3] = time.mktime(datetime.datetime.strptime(_data[len(_data) - 1][3], "%d.%m.%Y").timetuple())
        data = _data
